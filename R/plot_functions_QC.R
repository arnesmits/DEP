...
plot_normalization <- function(raw, norm) {
    ...
    df1$var <- as.character(substitute(raw))
    df2 <- assay(norm) %>%
      data.frame() %>%
      gather(ID, val) %>%
      left_join(., data.frame(colData(norm)), by = "ID")
    df2$var <- as.character(substitute(norm))

    df <- rbind(df1, df2)
    df$var <- parse_factor(df$var, levels = c(as.character(substitute(raw)),
                                              as.character(substitute(norm))))
