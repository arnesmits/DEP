...
merge_ibaq <- function(data, peptides) {
  ...
  if (length(grep("iBAQ.", colnames(data))) < 1) {
    stop("'iBAQ' columns are not present in data", call. = FALSE)
  }
...
#'
#' @param data SummarizedExperiment, Data object generated by \code{\link{linear_model}} and \code{\link{cutoffs}}.
#' @param ibaq Data.frame, iBAQ object generated by \code{\link{merge_ibaq}}.
...
stoichiometry <- function(data, ibaq, contrast, bait, level = 1) {
  ...
  if (length(grep("name_", colnames(ibaq))) < 1) {
    stop("merge information is not present in ibaq;\nRun ibaq_merge() to obtain the required data", call. = FALSE)
  }
...
#'
#' @param data SummarizedExperiment object, Data object which has been generated by generated by \code{\link{linear_model}} and \code{\link{cutoffs}}.
...
#' plot_ibaq(sign, "GFP_vs_WT", labelsize = 3)
#'
#' @export
plot_ibaq <- function(data, contrast, labelsize = 3) {
  ...
  if (length(grep("iBAQ.", colnames(rowData(data)))) < 1) {
    stop("iBAQ columns not present in data", call. = FALSE)
  }
  ...
  row_data$sign <- row_data[,col_sign]
  final <- row_data %>% select(name, lfc, Peptides, sign) %>% left_join(., stoi, by = "name")

  extra <- final %>% filter(sign == "+")
