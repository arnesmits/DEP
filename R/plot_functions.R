...
#' example_sign <- cutoffs(example_lm, alpha = 0.05, lfc = 1)
#'
#' plot_volcano(example_sign, "Ubi6_vs_Ctrl", labelsize = 5, add_names = TRUE)
#' plot_volcano(example_sign, "Ubi6_vs_Ctrl", add_names = FALSE)
#' plot_volcano(example_sign, "Ubi4_vs_Ctrl", labelsize = 5, add_names = TRUE)
...
#' @param norm SummarizedExperiment, Data object after normalization by \code{\link{norm_vsn}}.
...
#'
#' \code{plot_imp} generates density plots for all conditions before and after imputation
#'
#' @param raw SummarizedExperiment, Data object before imputation
#' @param imp SummarizedExperiment, Data object after imputation by \code{\link{imputation}}.
...
#' example_filter <- filter_missval(example_se, thr = 0)
#' example_vsn <- norm_vsn(example_filter)
#' example_imp <- imputation(example_vsn, "MinProb")
#'
#' plot_imp(example_vsn, example_imp)
#' @export
plot_imp <- function (raw, imp)
{
  # Show error if inputs are not the required classes
  ...
  if (any(!c("label", "ID", "condition", "replicate") %in% colnames(colData(imp)))) {
    stop("'label', 'ID', 'condition' and/or 'replicate' columns are not present in imp;\nRun make_se() or make_se_parse() to obtain the required data", call. = FALSE)
  }
...
plot_detect <- function(data) {
  ...
  if (any(!c("label", "ID", "condition", "replicate") %in% colnames(colData(data)))) {
    stop("'label', 'ID', 'condition' and/or 'replicate' columns are not present in data;\nRun make_se() or make_se_parse() to obtain the required data", call. = FALSE)
  }
