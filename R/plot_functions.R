...
plot_single <- function(data, protein, type) {
    ...
    if(length(which(rowData(data)$name == protein)) == 0) {
        possibilities <-
          rowData(data)$name[grep(substr(protein, 1, nchar(protein) - 1),
                                  rowData(data)$name)]
...
plot_heatmap <- function(data, type, k = 6, col_limit = 6, labelsize = 10) {
    ...
    # Plot the heatmap
    ht1 = Heatmap(df,
                  col = circlize::colorRamp2(seq(-col_limit, col_limit, (col_limit/5)),
...
plot_numbers <- function(data) {
    ...
    ggplot(stat, aes(x = ID, y = sum, fill = condition)) +
      ...
      labs(title = "Proteins per sample", x = "", y = "Number of ProteinGroups") +
      theme_DEP2()
}
