...
#' @return A barplot generated by \code{\link{ggplots2}}
#' @examples
...
#' @return A heatmap generated by \code{\link{ComplexHeatmap}}
#' @examples
#' example <- UbIA_MS %>% filter(Reverse != "+", Potential.contaminant != "+")
#' example_unique <- unique_names(example, "Gene.names", "Protein.IDs", delim = ";")
#'
...
#' example_sign <- cutoffs(example_lm, alpha = 0.05, lfc = 1)
#'
#' plot_heatmap(example_sign, "centered", k = 6, col_limit = 4)
#' plot_heatmap(example_sign, "contrast", k = 6, col_limit = 10)
#' @export
...
#' @return A volcano plot generated by \code{\link{ggplots2}}
#' @examples
#' example <- UbIA_MS %>% filter(Reverse != "+", Potential.contaminant != "+")
#' example_unique <- unique_names(example, "Gene.names", "Protein.IDs", delim = ";")
#'
...
#' example_sign <- cutoffs(example_lm, alpha = 0.05, lfc = 1)
#'
#' volcano(example_sign, "X6Ubi_.Con_", labelsize = 5, add_names = T)
#' volcano(example_sign, "X6Ubi_.Con_", add_names = F)
#' volcano(example_sign, "X4Ubi_.Con_", labelsize = 5, add_names = T)
#' volcano(example_sign, "X4Ubi_.Con_", add_names = F)
#' @export
...
#'
#' \code{plot_norm} generates boxplots for all samples before and after normalization.
#'
...
#' @return Boxplots for all samples before and after normalization generated by \code{\link{ggplots2}}
#' @examples
#' example <- UbIA_MS %>% filter(Reverse != "+", Potential.contaminant != "+")
#' example_unique <- unique_names(example, "Gene.names", "Protein.IDs", delim = ";")
#'
...
plot_norm <- function(raw, norm) {
  ...
  df <- rbind(df1, df2)
  ggplot(df, aes(x = ID, y = val, fill = sample)) + geom_boxplot(notch = T, na.rm=TRUE) + coord_flip() + facet_wrap(~var, ncol = 1) + theme_bw() + labs(x = "", y = "Log2 Intensity") +
...
#'
#' @param data ExpressionSet, Data object which has been generated by \code{\link{linear_model}} and \code{\link{cutoffs}}.
#' @return A heatmap indicating whether values are missing (0) or not (1). Generated by \code{\link{ComplexHeatmap}}
#' @examples
#' example <- UbIA_MS %>% filter(Reverse != "+", Potential.contaminant != "+")
#' example_unique <- unique_names(example, "Gene.names", "Protein.IDs", delim = ";")
#'
...
#'
#' \code{plot_numbers} generates a barplot of the number of identified proteins per samples.
#'
#' @param data ExpressionSet, Data object for which to plot protein numbers.
#' @return Barplot of the number of identified proteins per samples generated by \code{\link{ggplots2}}
#' @examples
#' example <- UbIA_MS %>% filter(Reverse != "+", Potential.contaminant != "+")
#' example_unique <- unique_names(example, "Gene.names", "Protein.IDs", delim = ";")
#'
...
plot_numbers <- function(data) {
  ...
  ggplot(stat, aes(x = ID, y = sum, fill = sample)) + geom_bar(stat = "identity") + theme_bw() + labs(title = "Proteins per sample", x = "", y = "Number of ProteinGroups") + geom_hline(yintercept = unique(stat$n)) +
...
#' @return Barplot of the number of samples proteins are identified generated by \code{\link{ggplots2}}
#' @examples
#' example <- UbIA_MS %>% filter(Reverse != "+", Potential.contaminant != "+")
#' example_unique <- unique_names(example, "Gene.names", "Protein.IDs", delim = ";")
#'
...
plot_frequency <- function(data) {
  ...
  ggplot(table, aes(x = Var1, y = Freq)) + geom_bar(stat = "identity") + theme_bw() + labs(title = "Protein identifications overlap between samples", x = "Identified in number of samples", y = "Number of ProteinGroups") +
