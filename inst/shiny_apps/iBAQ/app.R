		...
		dashboardSidebar(
		  sidebarMenu(
		    menuItem("Files", selected = TRUE,
		      ...
		      fileInput('file3', 'Peptides.txt',accept=c('text/csv', 'text/comma-separated-values,text/plain', '.csv')),
			...
			fluidRow(
			  column(width = 7,
			    tabBox(title = "Tables", width = 12,
			      tabPanel(title = "Top Table", box(uiOutput("select"), width = 6), box(uiOutput("exclude"), width = 6), DT::dataTableOutput("table")),
			      tabPanel(title = "Stoichiometry", DT::dataTableOutput("stoi_table"))
			    )
			  ),
			  column(width = 5,
  				tabBox(title = "Result Plots", width = 12,
  				  ...
  				  tabPanel(title = "Stoichiometry plot",
  				           fluidRow(
  				             ...
  				             box(numericInput("thr", "Threshold", min = 0, max = 1, value = 0.01), width = 4)
  				           ),
  				           fluidRow(
  				             plotOutput("stoi", height = 600),
  				             downloadButton('downloadStoi', 'Download stoichiometry')
  				           )
  				  )
				  ),
...
server <- shinyServer(function(input, output) {
  ...
  data <- reactive({
    ...
    if (is.null(inFile))
      return(NULL)
    data <- read.csv(inFile$datapath, header=T, sep="\t", stringsAsFactors = F)
  })
  expdesign <- reactive({
    inFile <- input$file2
    if (is.null(inFile))
      return(NULL)
    read.csv(inFile$datapath, header=T, sep="\t", stringsAsFactors = F)
  })
  peptides <- reactive({
    inFile <- input$file3
    if (is.null(inFile))
  ...
  observeEvent(input$analyze, {
    ...
    observeEvent(input$stoichiometry, {
      ...
      output$stoi_bait <- renderUI({
        if (!is.null(ibaq()) & !is.null(sign()) & !is.null(input$stoi_cntrst)) {
          row_data <- rowData(sign())
          ibaq <- ibaq()
