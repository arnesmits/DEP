...
* tested contrasts = <span style="color:blue">`r gsub("_p.adj", "", colnames(rowData(sign))[grep("p.adj", colnames(rowData(sign)))])`</span>  
...
```{r,echo=FALSE}
df <- assay(sign[rowData(sign)$sign == "+"]) - rowMeans(assay(sign[rowData(sign)$sign == "+"]))
...
plot_heatmap(sign, "contrast", k = 6, col_limit = (2 * col_lim))
```
...
plot_heatmap(sign, "centered", k = 6, col_limit = col_lim)
```
