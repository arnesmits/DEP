...
test_that("get_stoichiometry throws error without valid input", {
  expect_error(get_stoichiometry("test_ibaq_sign", test_ibaq_ibaq, "GFP_vs_WT", "Rbbp4", 1))
  expect_error(get_stoichiometry(test_ibaq_sign, "test_ibaq_ibaq", "GFP_vs_WT", "Rbbp4", 1))
  expect_error(get_stoichiometry(test_ibaq_sign, test_ibaq_ibaq, GFP_vs_WT, "Rbbp4", 1))
  expect_error(get_stoichiometry(test_ibaq_sign, test_ibaq_ibaq, "GFP_vs_WT", Rbbp4, 1))
  expect_error(get_stoichiometry(test_ibaq_sign, test_ibaq_ibaq, "GFP_vs_WT", "Rbbp4", "1"))
  expect_error(get_stoichiometry(test_ibaq_sign, test_ibaq_ibaq[,-(2:3)], "GFP_vs_WT", "Rbbp4", 1))
  expect_error(get_stoichiometry(test_ibaq_sign, test_ibaq_ibaq[,-(4:9)], "GFP_vs_WT", "Rbbp4", 1))
...
test_that("get_stoichiometry returns a data.frame", {
  expect_is(get_stoichiometry(test_ibaq_sign, test_ibaq_ibaq, "GFP_vs_WT", "Rbbp4", 1), "data.frame")
})

test_that("get_stoichiometry returns an object with the rigth dimensions and columns", {
  result <- get_stoichiometry(test_ibaq_sign, test_ibaq_ibaq, "GFP_vs_WT", "Rbbp4", 1)
